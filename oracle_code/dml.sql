...
-- CREATE SEQUENCE inv_id START WITH 1;
...
-- SELECT CASE WHEN :NEW.COUP_ID IS NOT NULL THEN 
--         CASE WHEN ((:NEW.ODO_END-:NEW.ODO_START)) > (:NEW.ODO_LIMIT)) THEN ROUND((VC.RATE*(:NEW.DROPOFF_DATE-:NEW.PICKUP_DATE))+(VC.OVER_FEE*((:NEW.ODO_END-:NEW.ODO_START))-:NEW.ODO_LIMIT)  - ((C.COUP_PERCENT * (VC.RATE*(:NEW.DROPOFF_DATE-:NEW.PICKUP_DATE))+(VC.OVER_FEE*((:NEW.ODO_END-:NEW.ODO_START))-:NEW.ODO_LIMIT))/100), 2)
--         ELSE ROUND((VC.RATE*(:NEW.DROPOFF_DATE-:NEW.PICKUP_DATE))  - ((C.COUP_PERCENT * (VC.RATE*(:NEW.DROPOFF_DATE-:NEW.PICKUP_DATE)))/100), 2) END
--       ELSE CASE WHEN ((:NEW.ODO_END-:NEW.ODO_START)) > (:NEW.ODO_LIMIT)) THEN ROUND((VC.RATE*(:NEW.DROPOFF_DATE-:NEW.PICKUP_DATE))+(VC.OVER_FEE*((:NEW.ODO_END-:NEW.ODO_START)-:NEW.ODO_LIMIT)), 2)
...
-- -- insert into na_invoice values(name_of_sequence.nextval,SYSDATE, 1.23, 1)

--  delete from na_booking;
-- delete from na_invoice;
-- -- select * from na_booking;
-- -- -- select * from na_coup;
-- INSERT INTO na_booking (booking_id, pickup_locid, pickup_date, dropoff_locid, dropoff_date, odo_start, odo_end, odo_limit, vin, cust_id, coup_id) VALUES( 1, 1, TO_DATE('01/01/2022 00:00:00', 'MM/DD/YYYY HH24:MI:SS'),1, TO_DATE('01/05/2022 00:00:00', 'MM/DD/YYYY HH24:MI:SS'),3540, 3582, 100, '1L2K3J4H123K4JH22', 2, NULL);
-- SELECT * FROM NA_INVOICE;
